...
export function obfuscateSelector({ seed, hashLength = HASH_LENGTH, hashAlgo = HASH_ALGO } = {}) {
  return (selector) => {
    ...
    ({ seed, hashLength = HASH_LENGTH, hashAlgo = HASH_ALGO } = {}) =>
      (root) => {
        root.walkRules((rule) => {
          ...
          rule.selectors = rule.selectors.map(obfuscateSelector({ seed, hashLength, hashAlgo }));
        });
      },
  );
}
