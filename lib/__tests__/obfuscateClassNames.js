...
describe('obfuscateClassNames', () => {
  describe('.obfuscateClassName', () => {
    it('hashes class name', () => {
      const actual = obfuscateClassName('App', { key });
  ...
  describe('.obfuscateSelector', () => {
    it('hashes complex selectors', () => {
      const obfuscated = _(['App', 'Component', 'state'])
        .map((className) => [className, obfuscateClassName(className, { key })])
      .fromPairs()
      .value();
      const actual = obfuscateSelector('.App .Component[attr=\'val\'].state:hover', { key });
